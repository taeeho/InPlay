"""학습된 LightGBM Booster → ONNX. Python(lgbm) vs ONNX runtime parity 검증.

산출:
- {onnx_path}: ONNX 모델 파일
- {sample_path}: deterministic input + lgbm_proba + onnx_proba + sha256.
  Java parity test가 이 sample을 읽어 Java ONNX 추론 결과와 비교.
"""
...
def _extract_proba(outputs) -> np.ndarray:
    # convert_lightgbm(zipmap=False) binary: outputs[1] shape (N,2). class1 = [:, 1].
    for out in outputs:
        arr = np.asarray(out)
...
def _deterministic_sample(n_features: int, count: int) -> np.ndarray:
    ...
    return rng.uniform(0.3, 0.7, size=(count, n_features))
